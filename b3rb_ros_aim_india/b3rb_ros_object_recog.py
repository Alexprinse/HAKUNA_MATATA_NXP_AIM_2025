...
class ObjectRecognizer(Node):
	...
	def __init__(self):
		...
		self.temporal_buffer = []  # List of dicts: each dict is {object_name: count, ...}
		self.temporal_buffer_size = 7  # Number of frames to buffer
		self.temporal_timer = None
		self.temporal_wait_seconds = 2.0  # Wait time for smoothing (seconds)
		self.temporal_collecting = False

	def start_temporal_smoothing(self):
		if self.temporal_collecting:
			return  # Already collecting
		self.temporal_buffer = []
		self.temporal_collecting = True
		if self.temporal_timer:
			self.destroy_timer(self.temporal_timer)
		self.temporal_timer = self.create_timer(self.temporal_wait_seconds, self.temporal_smoothing_callback)
		self.get_logger().info(f"Started temporal smoothing for {self.temporal_wait_seconds} seconds.")

	def temporal_smoothing_callback(self):
		# Called after wait period; process buffer and publish mode result
		if self.temporal_timer:
			self.destroy_timer(self.temporal_timer)
			self.temporal_timer = None
		self.temporal_collecting = False
		if not self.temporal_buffer:
			self.get_logger().warn("No detections collected during smoothing period.")
			return
		# Aggregate all object counts
		from collections import Counter, defaultdict
		object_counter = defaultdict(list)
		for det in self.temporal_buffer:
			for obj, count in det.items():
				object_counter[obj].append(count)
		# Take mode (most common count) for each object
		final_counts = {}
		for obj, counts in object_counter.items():
			try:
				from statistics import mode
				final_counts[obj] = mode(counts)
			except Exception:
				final_counts[obj] = max(set(counts), key=counts.count)
		...
		self.get_logger().info(f"Published temporally smoothed detection: {dict(final_counts)}")
	...
	def publish_debug_image(self, publisher, image):
		if image.size:
			...
			# self.get_logger().info(f"Published debug image on {publisher.topic_name if hasattr(publisher, 'topic_name') else 'unknown topic'} with shape {image.shape} and dtype {image.dtype}")
		else:
			self.get_logger().warn("Attempted to publish empty debug image.")
	...
	def camera_image_callback(self, message):
		...
		# --- Image pre-processing with CLAHE for contrast enhancement ---
		...
		if self.temporal_collecting:
			...
			self.temporal_buffer.append(object_count_dict.copy())
			# Limit buffer size
			if len(self.temporal_buffer) > self.temporal_buffer_size:
				self.temporal_buffer.pop(0)
		else:
			# If not collecting, publish immediately (legacy behavior)
			shelf_objects_message = WarehouseShelf()
	...
	# Optionally, you can call self.start_temporal_smoothing() externally when robot aligns to shelf
